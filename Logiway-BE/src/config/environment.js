...
function parseOverpassUrls(environment) {
  const configured = environment.OVERPASS_URLS?.trim() ||
    environment.OVERPASS_URL?.trim();
...
export function loadConfig(environment = process.env) {
  return {
    ...
    graphHopperUrl:
      environment.GRAPHHOPPER_URL?.trim() || DEFAULT_GRAPHHOPPER_URL,
